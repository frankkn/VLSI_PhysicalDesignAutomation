...
void Parser::readPl(string const &plPath)
{
  ...
  for(int i = 0; i < cellList.size(); ++i) 
  {
    ...
    NodeTable.at(cell_name)->x_global = cell_x;
    NodeTable.at(cell_name)->y_global = cell_y;
  }
  for(int i = 0; i < terminalList.size(); ++i) 
  {
    ...
    NodeTable.at(terminal_name)->x_global = terminal_x;
    NodeTable.at(terminal_name)->y_global = terminal_y;
  }
}
